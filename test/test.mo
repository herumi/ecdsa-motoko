...
func serializeTest() {
  ...
  let (x, y) = Option.get(M.deserializeCompressed(v.vals()), (0, 0));
  assert((x, y) == pub);
  let pub2 = (x, M.fpNeg(y));
  v := M.serializeCompressed(pub2);
  let pub3 = Option.get(M.deserializeCompressed(v.vals()), (0, 0));
  assert(pub3 == pub2);
};
